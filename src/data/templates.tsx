...
export const TEMPLATES: TemplateData[] = [
  ...
  { id: 't51', name: '雅致水墨', cat: 'art light', desc: '淡雅灰度 · 水墨', render: (cfg: CoverConfig) => (
    <div className="c c51"><div className="c51-ink" /><div className="c51-wrap" style={getTextLayoutStyle(cfg)}>{cfg.mainTitle.visible && <div style={getMainTitleContainerStyle(cfg)}><div className="c51-title" style={{...getTitleStyle(cfg), ...getMultilineTextStyle()}}>{cfg.mainTitle.content || '请输入文章标题'}</div></div>}<div className="c51-line" />{cfg.subtitle.visible && <div className="c51-sub" style={{...getSubtitleStyle(cfg), ...getMultilineTextStyle()}}>{cfg.subtitle.content || '副标题文字'}</div>}</div></div>
  )},
